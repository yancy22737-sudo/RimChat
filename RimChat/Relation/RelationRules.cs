    ...
    public class RelationRules
    {
        ...
        public string BasePath
        {
            get
            {
                ...
                string fallbackDir = System.IO.Path.Combine(GenFilePaths.SaveDataFolderPath, "RimChat", "rules");
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                _isInitialized = true;
                Log.Message($"[RimChat] RelationRules initialized, config path: {ConfigFilePath}");
            }
            catch (Exception ex)
            {
                Log.Error($"[RimChat] Failed to initialize RelationRules: {ex}");
        ...
        public RelationRulesConfig LoadConfig()
        {
            try
            {
                ...
                if (System.IO.File.Exists(ConfigFilePath))
                {
                    ...
                    if (config != null)
                    {
                        ...
                        Log.Message($"[RimChat] Loaded RelationRulesConfig from file");
                        return config;
                    }
                }

                Log.Message($"[RimChat] RelationRules config not found, creating default config");
                _cachedConfig = CreateDefaultConfig();
            ...
            {
                Log.Error($"[RimChat] Failed to load RelationRules config: {ex}");
        ...
        public void SaveConfig(RelationRulesConfig config)
        {
            try
            {
                ...
                if (config == null)
                {
                    Log.Warning("[RimChat] Attempted to save null RelationRules config");
                    return;
                }
                ...
                Log.Message($"[RimChat] Saved RelationRulesConfig to: {ConfigFilePath}");
            }
            catch (Exception ex)
            {
                Log.Error($"[RimChat] Failed to save RelationRules config: {ex}");
            }
        }
        ...
        public void ResetToDefault()
        {
            try
            {
                _cachedConfig = CreateDefaultConfig();
                SaveConfig(_cachedConfig);
                Log.Message("[RimChat] Reset RelationRulesConfig to default");
            }
            catch (Exception ex)
            {
                Log.Error($"[RimChat] Failed to reset RelationRules config: {ex}");
            }
        }
        ...
        private void EnsureDirectoryExists()
        {
            try
            {
                if (!System.IO.Directory.Exists(BasePath))
                {
                    ...
                    Log.Message($"[RimChat] Created RelationRules directory: {BasePath}");
                }
            }
            catch (Exception ex)
            {
                Log.Error($"[RimChat] Failed to create RelationRules directory: {ex}");
            }
        }
        ...
        private RelationRulesConfig ParseJsonToConfig(string json)
        {
            ...
            {
                Log.Warning($"[RimChat] Failed to parse RelationRules JSON: {ex.Message}");
                return null;
            }
        }
