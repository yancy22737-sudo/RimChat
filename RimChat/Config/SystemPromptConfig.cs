 ...
 ///</summary>
        private void InitializeMinimalDefaults()
        {
            ...
            ApiActions = new List<ApiActionConfig>
            {
                ...
                new ApiActionConfig("pay_prisoner_ransom", "Pay ransom for a player-held prisoner to the prisoner's source faction. System negotiates and can counter-offer before settlement.", "target_pawn_load_id (int, REQUIRED), offer_silver (int>0, REQUIRED), payment_mode (string, optional; omit or set exactly silver)", "Only for player-held prisoners that belong to current faction. Negotiation is system-driven (max 3 rounds, floor 0.8x). If target_pawn_load_id is missing or invalid, call request_info(info_type=prisoner) to request selection; otherwise pay_prisoner_ransom may be called directly. offer_silver must stay inside the current offer window shown by system messages. If a system message provides current ask, offer_silver must equal that current ask and must not reuse stale values from earlier memory. payment_mode may be omitted; if provided it must be exactly silver (valid: pay_prisoner_ransom,target_pawn_load_id:12345,offer_silver:1800,payment_mode:silver; invalid: pay_prisoner_ransom,target_pawn_load_id:12345,offer_silver:1800,payment_mode:cash)."),
            ...
            ResponseFormat = new ResponseFormatConfig
            {
                ...
                ImportantRules = "1. Match the user's game language while keeping JSON keys and action names unchanged.\n2. Use only enabled actions and obey requirements, cooldowns, and limits.\n3. Keep natural-language dialogue fully in character and follow the single authority contract in response_contract.\n4. Mirror the player's brevity when helpful, but keep enough clarity and tone to stay coherent.\n5. If intent is ambiguous, probe cautiously before escalating to suspicion or hostility.\n6. If no gameplay effect is intended, omit JSON entirely.\n7. Use request_info(info_type=prisoner) only when ransom target information is missing.\n8. If target_pawn_load_id is already known and valid, pay_prisoner_ransom may be called directly.\n9. For pay_prisoner_ransom, never claim payment/submission unless target_pawn_load_id and offer_silver are both valid.\n10. For pay_prisoner_ransom, payment_mode may be omitted; if provided, use exactly silver.\n11. For pay_prisoner_ransom, keep offer_silver within the current offer window provided by system messages.\n12. HARD RULE for pay_prisoner_ransom: when system messages provide current ask, offer_silver must equal current ask and must not reuse stale offers from memory."
            };
