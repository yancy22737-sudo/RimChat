 ...
 ///</summary>
        private void InitializeMinimalDefaults()
        {
            ...
            ApiActions = new List<ApiActionConfig>
            {
                ...
                new ApiActionConfig("request_item_airdrop", "Resolve one real in-game ThingDef from need text and send it by vanilla drop pod after player confirmation.", "need (string, required), budget_silver (int, required), scenario (string, optional: trade/ransom/general), constraints (string, optional), payment_items (array<object>, required; each: item (string), count (int>0))", "Need, budget_silver, and payment_items are required. payment_items must be valid beacon-tradable goods; total value must be >= budget and overpay must stay within 5%. Fail fast on any validation error."),
