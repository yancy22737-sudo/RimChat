 ...
 ///</summary>
        private void InitializeMinimalDefaults()
        {
            ...
            ApiActions = new List<ApiActionConfig>
            {
                ...
                new ApiActionConfig("pay_prisoner_ransom", "Submit a silver ransom payment for a player-held prisoner and register the ransom contract; prisoner release is handled manually by the player.", "target_pawn_load_id (int, REQUIRED), offer_silver (int>0, REQUIRED), payment_mode (string, optional; omit or set exactly silver)", "Only for player-held prisoners that belong to current faction. If target_pawn_load_id is missing or invalid, call request_info(info_type=prisoner) to request selection; otherwise pay_prisoner_ransom may be called directly. offer_silver must stay inside the current offer window shown by system messages. payment_mode may be omitted; if provided it must be exactly silver (valid: pay_prisoner_ransom,target_pawn_load_id:12345,offer_silver:1800,payment_mode:silver; invalid: pay_prisoner_ransom,target_pawn_load_id:12345,offer_silver:1800,payment_mode:cash). In normal flow, execute one payment submit only; in [RansomBatchSelection] flow, if you emit pay_prisoner_ransom in this turn you must output one action per listed target exactly once in the same response. Read [RansomState] before writing ransom-completion wording. MUST: if natural language claims ransom submitted/paid/settled/released, the same response must include pay_prisoner_ransom action."),
            ...
            ResponseFormat = new ResponseFormatConfig
            {
                ...
                ImportantRules = "1. Match the user's game language while keeping JSON keys and action names unchanged.\n2. Use only enabled actions and obey requirements, cooldowns, and limits.\n3. Keep natural-language dialogue fully in character and follow the single authority contract in response_contract.\n4. Mirror the player's brevity when helpful, but keep enough clarity and tone to stay coherent.\n5. If intent is ambiguous, probe cautiously before escalating to suspicion or hostility.\n6. Dialogue happens over the comms terminal, not an in-person/offline meeting. Do not describe arriving on-site, hand-to-hand transfer, or physically taking the prisoner away in this channel.\n7. If no gameplay effect is intended, omit JSON entirely.\n8. Use request_info(info_type=prisoner) only when ransom target information is missing.\n9. If target_pawn_load_id is already known and valid, pay_prisoner_ransom may be called directly.\n10. For pay_prisoner_ransom, never claim payment/submission unless target_pawn_load_id and offer_silver are both valid.\n11. For pay_prisoner_ransom, payment_mode may be omitted; if provided, use exactly silver.\n12. For pay_prisoner_ransom, keep offer_silver within the current offer window provided by system messages.\n13. For pay_prisoner_ransom, normal flow uses single submit; when [RansomBatchSelection] exists and you emit pay_prisoner_ransom, output one action for each listed target exactly once in the same response and keep total offer within the batch window.\n14. MUST: if natural language claims ransom paid/submitted/settled/released, the same response must include pay_prisoner_ransom action; otherwise rewrite into pending/clarification wording.\n15. You MUST read [RansomState] as the source of truth for paid/unpaid/pending-release status before writing ransom progress text.\n16. If [RansomState] current_request_paid=false, do NOT claim payment submitted/settled/released; ask for clarification or output the required action instead.\n17. If [RansomState] has_unpaid_ransom_request=true or pending_release_prisoner_count>0, avoid repeating 'just paid' claims unless the same response includes matching pay_prisoner_ransom action."
            };
