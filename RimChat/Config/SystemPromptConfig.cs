 ...
 ///</summary>
        private void InitializeMinimalDefaults()
        {
            ...
            ApiActions = new List<ApiActionConfig>
            {
                ...
                new ApiActionConfig("request_item_airdrop", "Resolve one real in-game ThingDef from need text and send it by vanilla drop pod.", "need (string, required), budget_silver (int, optional), scenario (string, optional: trade/ransom/general), constraints (string, optional)", "Need is required. Prefer natural-language need text and include multiple CN/EN aliases in constraints to improve hit rate. Fail fast if no valid ThingDef or no legal drop cell."),
