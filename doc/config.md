# RimChat 外部配置说明（v0.9.1024）

## 请求任务授勋禁用与冷却文案统一（v0.9.1024）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `create_quest` 可用任务集合移除 `BestowingCeremony`，请求任务菜单不再显示授勋仪式。
  - 当外部/旧提示词强行请求 `BestowingCeremony` 时，执行层会 fail-fast 返回 `bestowing_disabled`，并写入可检索日志锚点。
  - 外交 `[?]` 冷却提示统一按游戏内时间显示（天/时），不再按现实时间解释剩余秒数。
- 兼容说明：
  - 不改存档结构，不改冷却存储键，不改动作 JSON 外部键名。
  - 仅收敛任务模板可见性与冷却文案解释口径，避免空授勋请求污染后续授勋流程。

## `/models` 缺失回退与结构化原文直出（v0.9.1023）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - API 可用性诊断在 `/models` 返回 `404/405/501` 时，改为回退 chat 探测判定可用性，不再直接误判失败。
  - 本地 OpenAI 兼容探测在 `/v1/models` 缺失时不再直接判定服务离线，继续走 chat 最终验证。
  - RPG/外交结构化响应在重试耗尽后，不再注入固定沉浸回退句，改为原文直出。
  - RPG/外交响应契约最前新增 JSON 强约束前置示例，工作台预览可直接看到。
- 兼容说明：
  - 不改存档结构，不改动作 JSON 外部键名。
  - 仅收敛诊断判定与结构化失败回退策略，避免“接口可用却验证失败”和“固定文案吞回复”。

## 提示词页兼容渲染提示纠偏（v0.9.1020）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 当 CJK 语言环境下检测到默认 UGUI 字体不支持 CJK 时，提示词工作台仍会切换为 IMGUI 兼容渲染模式。
  - 切换兼容渲染时不再提示“页面渲染失败”，改为中性提示“已切换兼容渲染模式”。
  - 兼容渲染切换日志补充一次性诊断字段：语言目录、字体名、CJK 支持状态、UGUI 面板开关状态。
- 兼容说明：
  - 不改存档结构，不改提示词数据结构，不改外部动作 JSON 键名。
  - RimChat_PromptRenderFailed 仅保留给真实异常捕获路径使用。
## 赎金区间落点与社交新闻兼容修复（v0.9.1019）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 单目标赎金在选人后会生成会话级报价快照，并在每轮对话稳定注入 `[RansomOfferReference]`。
  - 单目标 `pay_prisoner_ransom` 的 `offer_silver` 若越界，将在执行前自动夹逼到最近边界值。
  - 批量赎金总价若越界，将自动夹逼到总窗口边界，并按原比例缩放分配到各目标（含整数余量修正）。
  - `social_circle_post` 原生渲染兼容失败改为“告警 + 降级继续”，不再直接中断社交新闻请求。
- 兼容说明：
  - 不改存档结构，不改动作外部 JSON 键名。
  - 仅收敛执行前归一化与新闻渲染失败策略，保证提示词/引用块/执行语义一致。
## 空投定价回退到市场价系统（v0.9.88）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投候选物资与交易卡需求参考价统一回退到市场价口径（最低 `0.01`）。
  - 空投预算派生继续使用市场价，并保留既有倍率规则：无 `tradeTags` 时 `x10`、`ExoticMisc` 时 `x2`。
  - 交易上限与运费规则保持不变。
- 兼容说明：
  - 不改存档结构与外部动作 JSON 键名。
  - 仅调整空投定价语义，保证 UI/提示词/执行一致。

## 空投交易规则统一与贸易价格口径根修（v0.9.87）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投交易分档规则改为固定策略：盟友商会 `150/12000`、非盟友商会 `200/800`、非商会盟友 `200/8000`、其他派系 `250` + 好感公式限额。
  - 空投预算派生与需求定价改为贸易买入价口径（受玩家谈判者社交技能影响），不再按 `MarketValue/BaseMarketValue`。
  - 缺少有效玩家谈判者或交易上下文（商队/轨道商）时，空投动作会 fail-fast 阻断。
  - 交易卡参考价和运费提示改为动态运费单价，不再固定显示 `500/仓`。
- 兼容说明：
  - 不改存档结构。
  - 不改空投动作外部 JSON 键名。
  - 仅收敛规则计算与价格来源，保证提示词/UI/执行口径一致。

## Google API 模型加载与配置校验根修（v0.9.79）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 当 provider 选择 `Google` 且 `Base URL` 留空时，模型列表拉取与快速连通性测试继续使用内置 Google 默认端点，不再误提示“URL 不能为空”。
  - `测试连通性` 现在只校验“是否存在已启用云端配置且已填写 API Key”，不再因为尚未选中模型而直接报“没有有效配置”。
  - `测试可用性` 仍然要求已选择模型，但当模型为空时会直接显示精确提示“模型不能为空，请先加载并选择模型”。
  - Google 模型列表解析增加 `name` 字段兜底抽取，减少服务端响应字段顺序/冗余字段变化导致的空模型菜单。
- 兼容说明：
  - 不改存档结构。
  - 不改运行时聊天请求主链。
  - 仅收敛 API 设置页的模型加载、快速连通性与可用性前置校验行为。

## 对话结构化主协议与思维链主链退出（v0.9.71）

- 本版本无新增用户可调业务开关。
- 固定行为变更：
  - RPG / 外交 / 主动推送的模型回复默认要求返回一个结构化 JSON 对象，主字段为 `visible_dialogue`，`actions` 仅在确有 gameplay 效果时出现。
  - 旧的“对白 + 尾随 actions JSON”只保留最小输入兼容，不再作为主协议。
  - 输出侧新增协议 fail-fast；若结构不合规，会先重试，再落本地安全回复，不再把不合规原文直通 UI。
  - thought-chain 默认对全部通道关闭；即使保留旧配置字段，也不再作为运行时安全边界。
- 兼容说明：
  - 不新增存档字段，不破坏旧存档读取。
  - 提示词工作台仍可看到 thought-chain 节点与配置，但默认不会进入主运行链路。

## 外交历史记录面板高保真重做（v0.9.70）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 外交历史窗口改为高保真 RPG 历史面板风格。
  - 不再区分多个视图，只展示当前外交派系的会话化历史。
  - 单击记录行选中，双击编辑，选中后右侧才显示删除符号。
  - 编辑与删除会同步修改当前外交会话消息和持久化历史。
...
- 固定行为变更：
  - 外交窗口顶部按钮行新增 `历史记录` 入口。
  - 历史记录窗口只管理外交 `DialogueHistory`，支持编辑消息文本和删除单条记录。
  - `玩家总历史` 为聚合视图，不新增独立存档字段。
- 数据约束：
  - 空文本编辑会直接 fail-fast 阻断，不会写入存档。
  - 编辑 / 删除后立即回写原派系 `LeaderMemory` 并持久化。

## 社交圈原生渲染兼容 fail-fast（v0.9.58）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 社交圈 `social_circle_post` 在原生 RimTalk 渲染签名不兼容时改为告警并继续发送降级 prompt，不再直接 fail-fast 中断。
...
- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 外交/RPG/推送通道在“解析最终失败”时改为 fail-fast 返回本地化错误，不再把固定兜底句写入会话历史。
  - 仅当模型返回“可识别结构但主文本为空”时，允许一次解析重试；其余解析失败不再重试。
  - 解析层新增 `content[]` 文本片段提取，提升本地模型 `127.0.0.1:4315` 回包兼容性。
- 调试观测：
  - 开启内部日志后可看到解析状态、原因标签、命中路径与内容长度，便于定位回包结构问题。

## 批量囚犯赎金谈判（v0.9.48）

- 本版本无新增用户可调配置项（批量宽松策略为固定默认值）。
- 固定行为变更：
  - 赎金囚犯选择窗口支持多选，默认全不选，并提供 `全选/全不选/确认`。
  - 选择多人后系统生成批量囚犯情报卡（逐人列表 + 总叫价 + 总报价区间），并进入批量赎金上下文。
  - 批量上下文下如果本轮执行 `pay_prisoner_ransom`，必须同轮覆盖全部已选目标，且总报价在总区间内；不满足会 fail-fast。
  - 批量执行为串行首错即停；已成功目标不回滚，后续仅需覆盖剩余目标。
  - 批量惩罚固定中度宽松：放人时限 +50%，普通超时/跌价惩罚系数 0.7，跌价阈值上调。
  - 器官新增缺失惩罚在批量模式下保持单人强度，不做宽松。

## 外交主动节流加强与恢复不补发（v0.9.46）

- 新增/强化用户可调配置项（NPC 主动设置）：
  ...
  - `EnableNpcPushThrottleDebugLog`
    - 默认值：`false`
    - 含义：开启后输出节流命中与恢复清队列调试日志。
- 固定行为变更：
  - 当派系在线状态从不可达恢复为在线时，历史主动队列会立刻清空，不再补发旧触发。
  - 正常外交主动链路默认三层节流：
    - 全局冷却：6 小时最多一条
    - 派系冷却：每次投递后 3~7 天
    - 队列上限：每派系最多 3 条，默认 12 小时过期
- 迁移策略：
  - 旧存档首次加载会强制迁移到上述节流默认档位，避免历史配置继续导致恢复后高频推送。

## 玩家手动社交圈发帖 + 派系强制主动回应（v0.9.44）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 玩家可在外交窗口 `社交圈` 页签中点击 `发帖/Post`，打开最小化发帖窗口。
  - 发帖窗口只允许填写标题和正文，不提供分类、目标派系、可信度等额外字段。
  - 标题或正文为空时会被直接阻断；标题和正文超过系统上限时也会 fail-fast 阻断。
  - 帖子发布后会立即进入社交圈列表，并按内容相关性自动触发 1-3 个有效派系主动来信。
  - 这些派系回应继续走现有主动外交链路，不新增独立配置开关。
...
- 默认行为：
  - 开启时，策略状态区展开显示完整状态文案（可用次数 / 社交门槛 / 生成中 / 已就绪 / 已用尽 + 关闭入口）。
  - 关闭时，策略状态区折叠为单行 `RimChat_StrategyCollapsedEntry` 入口，仅保留开启入口，不再显示次数文案。
- 布局行为：
  - 折叠/展开使用高度伸缩 + 淡入淡出动画。
  - 折叠后释放出的纵向空间回收到消息列表区域，而不是留在控制区内空白。
- 兼容规则：
  - 当策略按钮三选项已可显示时，仍然隐藏状态行，只显示策略按钮栏。
  - 不改变策略 follow-up 请求、策略按钮展示/点击发送与会话次数消耗的原有门控语义。
...
- 默认值：
  - `true`
- 生效范围：
  - 仅影响通讯台外交对话窗口内的策略提示行、策略 follow-up 请求与策略按钮展示/点击发送。
- 固定行为变更：
  - 当策略按钮三选项已可显示时，提示行自动隐藏，不再同时显示“策略可用次数/社交门槛”文字。
  - 当策略按钮未显示时，提示行尾部会追加可点击文案；开启状态显示“点击关闭”，关闭状态显示“点击开启”。
  - 关闭后不会再发起新的策略补请求，也不会显示或发送策略按钮；普通外交主对话链路不受影响。
...
- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `WoodLog` 等具备明确资源信号（`stuffProps`）的原材料，资源家族判定优先通过，不再被噪声 `IsWeapon` 误杀。
  - 当空投交易卡已绑定 `need_def` 且可解析时，文本需求类别漂移不会再阻断交易；系统改为记录审计并继续按绑定物资执行。
  - 仅在绑定物资无法解析时继续 fail-fast 阻断（`bound_need_unresolved`）。

## 空投聊天卡配色回调与截断修复（v0.9.23）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投聊天卡灰色内容层的文字配色重新调整，增强与背景的对比度。
  - 名称、`defName` 与指标值区域的高度预算上调，进一步减少截断。

## 空投聊天卡原气泡回调与灰色内容层（v0.9.22）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投聊天卡恢复原有玩家/AI 外层气泡配色。
  - 仅中间需求/出价内容层使用灰色底。
  - 文字与指标区高度预算回调，减少文本被裁切。

## 空投聊天卡标题层回调（v0.9.21）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 在单层灰底样式下，空投聊天卡标题会以更清晰的独立行保留显示。
  - 仍不恢复多层彩色底板和装饰条。
...
- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 聊天区空投交易卡改为单层灰底，不再使用多层彩色标题带、块底和强调条。
  - 卡内各区块改为细分隔线分层，整体高度进一步压缩。

## 空投聊天卡紧凑化与入口冷却阻断（v0.9.19）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 聊天区空投交易卡缩略图与指标区改为更紧凑布局，但仍保留 `defName`、数量、单价、总价。
  - `+发送信息 -> 发送空投交易请求` 现在会在打开信息卡前先检查 `request_item_airdrop` eligibility；若命中 `airdrop_cooldown`，菜单项直接显示剩余游戏内时间且不可点击。
  - 入口阻断与 `[?]` 提示共用同一份冷却校验和游戏内时间格式化逻辑。

## 空投卡视觉与 Presence 状态重置修正（v0.9.18）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 聊天窗口里的空投信息卡改为沉浸式终端单据样式，不影响提交载荷或确认流程。
  - 当交易卡发起轮次 AI 未执行 `request_item_airdrop` 时，前台不再显示“空投信息被忽略，无交易行为”系统消息。
  - `request_item_airdrop` 的受限提示改为显示游戏内剩余时长，不再按现实分钟/小时解释冷却。
  - `DoNotDisturb` 改为固定维持 `3` 个游戏日，到期后回到派系排班计算态。

## 空投重报价沉浸式解析升级（v0.9.17）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - AI 重报价不再需要输出生硬的固定模板，可以直接使用角色内自然对白。
  - 只要对白中清楚包含物资、数量、银币价格，系统就会缓存并用于下一次开卡默认回填。
  - 若对白省略了物资名，但当前交易卡仍绑定目标物资，系统会自动用绑定物资补齐解析。

## 空投确认数量来源纠偏（v0.9.16）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 当空投交易卡仍处于绑定状态时，最终确认数量优先使用交易卡结构化保存的需求数量。
  - 包含多个数字的玩家消息不再默认取最大值作为空投数量，避免把付款银币识别成物资数量。

## 原木资源资格误判修复（v0.9.15）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `WoodLog` 等具备明确资源信号的原材料，不再因为底层 `IsWeapon` 噪声标记被错误踢出资源候选池。
  - 绑定需求仲裁在资源类请求下会正确接受原木，不再误报“当前需求类别冲突”。

## 空投交易卡绑定物品状态贯通根修（v0.9.14）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 交易卡绑定的目标物资会跨回合保留，直到玩家成功提交、显式取消、重新提交新卡，或显式改选其他物资。
  - 普通“确认/发吧/下单”消息不再清空交易卡绑定状态。
  - 若系统检测到绑定状态丢失，或最终准备出的物资与交易卡绑定物资不一致，会直接阻断交易并提示重新提交交易卡。
  - AI 仍可重报价和调整数量/付款方式，但在绑定存在时不能静默替换目标物资。

## 空投绑定需求仲裁根修（v0.9.13）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投交易卡提交后，系统会把绑定的需求 `defName/label/searchText` 带入执行链路，而不再只依赖自由文本 `need`。
  - 若候选池与绑定需求冲突，系统会按绑定需求重建交易，不再把错误候选送进确认窗。
  - 若绑定需求本身无法解析，或与推断出的需求家族冲突，系统会前台阻断交易并记录详细诊断日志。
  - AI `Counteroffer/重报价` 继续只影响排序和默认值，不会覆盖绑定需求。

## 空投请求 UI/超时/匹配链路重构（v0.9.12）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投信息卡提交前必须精确绑定需求物资，未绑定时显示本地化阻断原因并禁止提交。
  - 参考价格固定按“需求物资市场价 x 需求数量”实时计算；需求未绑定时不显示伪造价格。
  - 二次开窗只保留出价区默认值，需求搜索框、需求卡和需求数量恢复默认状态。
  - 空投二阶段 `selection_timeout/queue_timeout` 默认自动取 Top1 并进入最终确认，不再请求玩家选择候选，也不再发送该阶段系统消息。
  - 物资搜索、支付解析与候选排序统一使用共享多维度匹配器；低置信度输入直接失败，不再进行模糊兜底成交。

## 空投信息卡可用性修复（v0.9.9）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 信息卡移除“候选/刷新/搜索”交互，改为“需求输入 + 以物易物库存选择 + 出价数量”。
  - 库存列表仅展示通电轨道信标可交易物资，并显示可用数量。
  - 当信标库存为空时，界面直接提示“无可用于以物易物的库存”。
  - 底部输入与按钮改为响应式布局，避免遮挡。
...
- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 空投信息卡提交后仅显示自然语言摘要，结构化参数改为内部注入到 AI 请求，不在聊天区直接显示。
  - 信息卡定位为“参考报价输入”，AI 可拒绝、可重报价、可改参数执行。
  - AI 文本重报价支持固定句式解析并会话缓存，下一次开卡默认回填 `item/count/silver`。
  - 若信息卡发起轮次 AI 未返回 `request_item_airdrop` 动作，系统提示“空投信息被忽略，无交易行为”。
  - 右侧库存列表改为“通电轨道信标可达物资 + 可用数量”。
  - `[?]` 提示受限原因改为 code 本地化优先，未命中回退 `validation.message`，再回退通用文案。

## 启动期 Harmony 补丁参数规范与自检（v0.9.6）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 关键启动期补丁参数改为位置参数规范（`__0/__1`）以降低参数命名漂移风险。
  - 启动时在 `PatchAll` 前执行一次最小补丁签名自检，并输出通过/失败日志。
  - 自检仅用于诊断，不改动用户配置。
...
- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 当当前语言缺少 RimChat keyed 语言包时，`RimChat_*` 键自动回退英文文本，避免键名直出/乱码。
  - 回退逻辑仅覆盖 RimChat 前缀键，不影响游戏本体或其他 Mod 的翻译键。
  - 首次命中回退时在日志中输出一次明确告警，便于问题定位。

## 版本日志语言目录直读与动态语言列表（v0.9.4）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 版本日志语言选择改为按 `LanguageDatabase.activeLanguage.folderName` 直读 `1.6/Languages/<folderName>` 同名目录。
  - 若直读失败，改为“归一化 + 别名映射”匹配语言目录（例如 `zh-Hans` -> `ChineseSimplified`）。
  - 若仍找不到语言目录，立即 fail-fast 回退 `English`，并在日志打印明确告警（包含当前语言、尝试结果、最终回退路径）。
  - `AvailableLanguages` 改为扫描 `1.6/Languages` 目录动态生成，不再硬编码仅中英。
  - 版本日志文件候选改为：优先 `VersionLog_<语言目录>.txt`，其次 `VersionLog.txt`（本地化默认），最后 `VersionLog_en.txt`。
  - 当语言专属版本日志不存在时，自动回退 `VersionLog_en.txt` 并打印明确告警。
...
- 新增可调项：
  - `ItemAirdropCooldownTicks`
    - 含义：空投交易冷却时间（ticks），按派系维度独立计算。
    - 默认：`180000`（3 天）
    - 约束：`60000..432000`（1~7 天）
    - 说明：仅在空投成功 commit 后触发；取消/失败/超时均不触发冷却。
- 行为变更：
  - `request_item_airdrop` 在 eligibility 与 `[?]` 提示阶段均会检查冷却，冷却期内返回 `airdrop_cooldown` 并附带 `RemainingSeconds`。
  - `+发送信息` 菜单新增"发送空投交易请求"，打开信息卡窗口直接提交结构化空投请求。
...
- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 发送囚犯信息卡时会记录核心器官快照（心/肝/肺/肾/眼），同囚犯后发覆盖前发。
  - 信息卡每次都会强制重算参考赎金；读档/开新局会清空赎金运行态缓存，避免沿用旧报价。
  - 囚犯离图时若出现“相对信息卡基准的新增核心器官缺失”，将延迟 5-10 游戏小时后判定离图失败并按超时级重罚。
  - 命中器官失败路径时不叠加即时价值缩水处罚，避免同一事件双罚。
- 兼容性：
  - 赎金合约新增器官快照/延迟失败相关持久化字段，全部带默认值，旧档可直接读取。

## 赎金合约健康离图回执与超时谴责（v0.8.19）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 囚犯离图后若满足严格健康（健康 >= 85%、意识 >= 85%、非倒地），会在 5-10 游戏小时后收到派系主动回执（会话消息 + 来信）。
  - 赎金超时未离图时，保留原有重罚/报复袭击逻辑，同时新增会话警告来信。
  - 超时同时触发派系首领社交圈谴责帖（走社交圈 AI 生成链路）。
- 兼容性：
  - 赎金合约记录新增持久化字段（目标名快照、延迟回执状态），全部带默认值，旧档可直接读取。

## 赎金超时冷却与重复选人去重（v0.8.14）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `request_info(info_type=prisoner)` 在会话已有有效绑定目标时会直接去重短路，不再重复弹窗选人。
  - 选人流程增加重入门禁；“正在选人中”的重复触发会被拒绝。
  - 囚犯信息卡自动回复链路命中超时后进入 90 秒冷却，冷却期只抑制自动重发，不抑制玩家手动发送。
  - 超时会被分类记录为 `queue_timeout / network_timeout / drop_timeout`，用于日志排查。
- 兼容性：
  - 不新增存档持久化字段，仅增加会话运行态冷却字段，不影响旧档读取。

## 赎金承诺动作一致性（MUST）强化（v0.8.13）

- 本版本无新增用户可调配置项。
- 固定规则强化：
  - 文本出现“已提交/已支付/钱货两清/已放人离开”等赎金完成态时，必须同条携带 `pay_prisoner_ransom` 动作。
  - 若当轮无该动作，模型必须使用待确认/待提交语气，不得宣称已完成。
  - 通信终端在线语境继续加强，禁止线下完成态措辞。

## 通信终端语境与赎金承诺动作一致性（v0.8.12）

- 本版本无新增用户可调配置项。
- 固定规则强化：
  - AI 对话语境固定为通信终端在线聊天，不是线下见面。
  - 若文本承诺“已提交/已支付赎金”，同条回复必须带 `pay_prisoner_ransom` 动作。
  - 该约束已在默认提示词、系统默认配置与运行时迁移补丁中统一生效。
- 日志定位：
  - 可在 `C:\\Users\\Administrator\\AppData\\LocalLow\\Ludeon Studios\\RimWorld by Ludeon Studios\\Player.log` 检查文本承诺与动作输出是否同轮一致。

## 赎金单次支付提交重构（v0.8.11）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `pay_prisoner_ransom` 改为单次支付提交，不再进行代码侧多轮议价。
  - 报价规则保持“落在当前可报价区间内即可”；不要求命中当前叫价。
  - 支付成功后仅表示“到账并登记合约”，放人由玩家手动执行。
  - 移除自动放人前置校验对支付的阻断（不再因 `release_cell_not_found` 失败）。
  - 失败提示统一为简洁系统原因（参数、目标、区间、支付模式、系统可用性）。

## 赎金非终态反馈可视化（v0.8.10）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `pay_prisoner_ransom` 返回 `counter_offer` 时，系统会追加可读还价反馈（报价、当前还价、轮次）。
  - 返回 `rejected_floor_not_met` 时，系统会追加可读终止反馈（最后报价、底价、目标）。
  - 行为仅增强显示层，不改变赎金议价规则与执行契约。

## 赎金报价回归区间校验（v0.8.9）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `pay_prisoner_ransom` 报价规则恢复为“只要位于当前可报价区间内即可”。
  - 当前叫价仅作为建议参考，不再是必须精确命中的执行门禁。
  - 越界报价仍会被 fail-fast 拒绝（窗口校验保持不变）。

## 赎金当前叫价硬校验（v0.8.8）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - 当系统存在当前叫价（`currentAsk`）时，`pay_prisoner_ransom` 的 `offer_silver` 必须等于当前叫价。
  - 报价仅“在窗口内”但不等于当前叫价时，会被直接拒绝并提示重新按当前叫价报价。
  - 失败提示包含 `offered/currentAsk/min/max`，用于快速修正下一轮报价。

## 赎金终态清理修复（v0.8.7）

- 本版本无新增用户可调配置项。
- 固定行为变更：
  - `pay_prisoner_ransom` 仅在终态成功 `accepted_and_released` 时清理赎金会话绑定状态。
  - `counter_offer` 与 `rejected_floor_not_met` 不再清理绑定，避免下一轮重复触发囚犯重选。
  - 仅在目标缺失/失效时才触发 `request_info(info_type=prisoner)` 选人补信息。
...
## 默认预设 `mod_variables` 清空收敛（v0.9.72）

- 默认语义收敛：
  - `mod_variables` 在默认预设与工作台中均保持空字符串；
  - 不再因设置加载、预设重建、通道切换或预览构建而自动回填变量列表。
- 工作台行为：
  - `mod_variables` 现在是纯手动编辑区；
  - 变量浏览器仍可用于手动插入 RimTalk raw token，但空白 section 不再自动显示变量清单。
- 运行时行为：
  - 若 `mod_variables` section 为空，运行时与增量预览都输出空段；
  - 不再为 RPG `mod_variables` 空白 section 注入自动生成的变量块。
...
- 填充来源范围：
  - 手动插入时仅使用 RimTalk 自定义变量 raw token，不包含 RimChat core/builtin 变量。
- 用户编辑语义：
  - `mod_variables` 默认保持空白；
  - 仅用户手动填写的内容会进入预览与运行时渲染。
- 桥接 fail-fast 行为：
...
- `PresenceForcedOfflineHours`
  - 保留该滑条用于配置界面连续性，不再影响 `go_offline/set_dnd` 强制时长运行态。
  - 当前强制时长已固定为：`go_offline=2小时`，`set_dnd=4小时`。
  - 默认：`24`。
...
- `set_dnd`：切换为请勿打扰并停止交互。
  - 两者均为固定强制时长；到期后立即恢复在线，不再先受排班离线判定阻塞。
...
- 无新增用户可配置项（不新增设置开关）。
- UI 行为调整：
  - 日志观测趋势区右侧新增“本局统计”面板，仅显示：
    - 平均每分钟调用
    - 平均每分钟令牌
    - 平均单次调用令牌
  - 记录表改为分页展示，分页按钮：
    - 首页 / 上一页 / 下一页 / 末页
  - 每页条数动态计算：按当前可视高度自动计算，不提供固定页大小配置。
- 数据口径：
  - 趋势图保持最近 30 分钟（1 分钟桶）。
  - 列表改为本次游戏进程完整记录（退出游戏后清空，不跨重启）。
