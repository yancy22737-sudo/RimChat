...
namespace RimDiplomacy.Config
{
    ...
    public partial class RimDiplomacySettings : ModSettings
    {
        ...
        public string RPGApiGuidelines = "You can trigger game effects by including a JSON block at the end of your response. Use 'TryGainMemory' to add thoughts, 'TryAffectSocialGoodwill' to change relations, 'ReduceResistance' or 'ReduceWill' for prisoners, 'Recruit' to join the colony, and 'TriggerIncident' to start a game event (e.g., RaidEnemy, TraderCaravanArrival).";
        ...
        public override void ExposeData()
        {
            ...
            Scribe_Values.Look(ref RPGApiGuidelines, "RPGApiGuidelines", "You can trigger game effects by including a JSON block at the end of your response. Use 'TryGainMemory' to add thoughts, 'TryAffectSocialGoodwill' to change relations, 'ReduceResistance' or 'ReduceWill' for prisoners, 'Recruit' to join the colony, 'TriggerIncident' to start a game event (e.g., RaidEnemy, TraderCaravanArrival), and 'CreateQuest' to offer a custom mission to the player.");
            Scribe_Values.Look(ref RPGFormatConstraint, "RPGFormatConstraint", "Please output a JSON code block after your text if you want to trigger any game effects or relationship changes.\n\nFormat:\n```json\n{\n  \"favorability_delta\": 0.0,\n  \"trust_delta\": 0.0, \n  \"fear_delta\": 0.0,\n  \"respect_delta\": 0.0,\n  \"dependency_delta\": 0.0,\n  \"actions\": [\n    { \"action\": \"TryGainMemory\", \"defName\": \"JoyFilled\" },\n    { \"action\": \"TriggerIncident\", \"defName\": \"RaidEnemy\", \"amount\": 500 },\n    { \"action\": \"CreateQuest\", \"title\": \"Rescue my friend\", \"description\": \"Help me save my friend from bandits.\", \"rewardDescription\": \"1000 Silver\", \"callbackId\": \"quest_001\" }\n  ]\n}\n```\nIMPORTANT: Use EXACTLY the format above. 'actions' must be an array of objects. Only include fields that have non-zero changes. If no effects occur, you may omit the JSON block.");
            ...
            if (Scribe.mode == LoadSaveMode.LoadingVars)
            {
                ...
                // 移除旧的任务分发 API 指引 (Migration to remove CreateQuest)
                if (string.IsNullOrEmpty(RPGApiGuidelines) || RPGApiGuidelines.Contains("CreateQuest")) 
                    RPGApiGuidelines = "You can trigger game effects by including a JSON block at the end of your response. Use 'TryGainMemory' to add thoughts, 'TryAffectSocialGoodwill' to change relations, 'ReduceResistance' or 'ReduceWill' for prisoners, 'Recruit' to join the colony, and 'TriggerIncident' to start a game event (e.g., RaidEnemy, TraderCaravanArrival).";
                
                if (string.IsNullOrEmpty(RPGFormatConstraint) || RPGFormatConstraint.Contains("CreateQuest")) 
