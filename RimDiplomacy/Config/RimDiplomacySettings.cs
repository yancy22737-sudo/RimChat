...
namespace RimDiplomacy.Config
{
    ...
    public partial class RimDiplomacySettings : ModSettings
    {
        ...
        public string RPGApiGuidelines = "You can trigger game effects by including a JSON block at the end of your response. Use 'TryGainMemory' to add thoughts (defName examples: Chitchat, DeepTalk, KindWords, Slighted, Insulted; do not use Chatted), 'TryAffectSocialGoodwill' to change relations, 'RomanceAttempt' to force-set lover state, 'MarriageProposal' to force-set spouse state, 'Breakup' to force-set breakup state, 'Divorce' to force-set ex-spouse state, 'Date' to advance romantic state, 'ReduceResistance' or 'ReduceWill' for prisoners, 'Recruit' to join the colony, 'TriggerIncident' to start a game event (e.g., RaidEnemy, TraderCaravanArrival), 'GrantInspiration' to attempt an inspiration, 'ExitDialogue' for normal closure with no cooldown, and 'ExitDialogueCooldown' to end the conversation and reject new chats for 3 hours.";
        ...
        public override void ExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.LoadingVars)
            {
                ...
                // 迁移旧版 RPG API 指引到当前动作集（含关系动作）
                if (string.IsNullOrEmpty(RPGApiGuidelines) || RPGApiGuidelines.Contains("CreateQuest") || !RPGApiGuidelines.Contains("ExitDialogueCooldown") || !RPGApiGuidelines.Contains("GrantInspiration") || !RPGApiGuidelines.Contains("RomanceAttempt") || !RPGApiGuidelines.Contains("Divorce")) 
