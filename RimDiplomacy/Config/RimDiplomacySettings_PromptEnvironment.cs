...
namespace RimDiplomacy.Config
{
    public partial class RimDiplomacySettings
    {
        ...
        {
            Rect contentRect = DrawEnvironmentCard(rect, "RimDiplomacy_EnvironmentWorldviewLabel");
            bool changed = false;
            bool oldEnabled = envConfig.Worldview.Enabled;

            Widgets.CheckboxLabeled(
                new Rect(contentRect.x, contentRect.y, contentRect.width, 24f),
                "RimDiplomacy_EnvironmentWorldviewEnabled".Translate(),
            ...
            Widgets.Label(
                ...
                "RimDiplomacy_EnvironmentCharCount".Translate(length, limit));
            GUI.color = Color.white;
        ...
        {
            Rect contentRect = DrawEnvironmentCard(rect, "RimDiplomacy_EnvironmentSceneSystemLabel");
            bool changed = false;
            ...
            Widgets.CheckboxLabeled(
                ...
                "RimDiplomacy_EnvironmentSceneSystemEnabled".Translate(),
            ...
            Widgets.CheckboxLabeled(
                new Rect(contentRect.x, y, contentRect.width, 24f),
                "RimDiplomacy_EnvironmentPresetTagsEnabled".Translate(),
            ...
            changed |= DrawEnvironmentIntSlider(
                ...
                200,
                4000,
                "RimDiplomacy_EnvironmentMaxSceneChars");
            ...
            changed |= DrawEnvironmentIntSlider(
                ...
                500,
                12000,
                "RimDiplomacy_EnvironmentMaxTotalChars");
        ...
        {
            Rect contentRect = DrawEnvironmentCard(rect, "RimDiplomacy_EnvironmentContextLabel");
            bool changed = false;
            ...
            Widgets.CheckboxLabeled(
                new Rect(contentRect.x, contentRect.y, contentRect.width, 24f),
                "RimDiplomacy_EnvironmentContextEnabled".Translate(),
        ...
        private bool DrawEnvironmentContextLeftColumn(Rect rect, EnvironmentContextSwitchesConfig switches)
        {
            bool changed = false;
            float y = rect.y;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextTime", ref switches.IncludeTime); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextDate", ref switches.IncludeDate); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextSeason", ref switches.IncludeSeason); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextWeather", ref switches.IncludeWeather); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextLocationTemperature", ref switches.IncludeLocationAndTemperature);
            return changed;
        }
        ...
        private bool DrawEnvironmentContextRightColumn(Rect rect, EnvironmentContextSwitchesConfig switches)
        {
            bool changed = false;
            float y = rect.y;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextTerrain", ref switches.IncludeTerrain); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextBeauty", ref switches.IncludeBeauty); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextCleanliness", ref switches.IncludeCleanliness); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextSurroundings", ref switches.IncludeSurroundings); y += 24f;
            changed |= DrawEnvironmentCheckbox(new Rect(rect.x, y, rect.width, 24f), "RimDiplomacy_EnvironmentContextWealth", ref switches.IncludeWealth);
            return changed;
        }
        ...
        {
            Rect contentRect = DrawEnvironmentCard(rect, "RimDiplomacy_EnvironmentSceneEntriesLabel");
        ...
        private bool DrawEnvironmentSceneList(Rect rect, EnvironmentPromptConfig envConfig)
        {
            ...
            if (Widgets.ButtonText(addRect, "RimDiplomacy_EnvironmentAddScene".Translate()))
            {
                envConfig.SceneEntries.Add(new ScenePromptEntryConfig
                {
                    Name = "RimDiplomacy_EnvironmentNewSceneName".Translate().ToString(),
                    Priority = 10,
            ...
            if (Widgets.ButtonText(removeRect, "RimDiplomacy_EnvironmentRemoveScene".Translate(), active: hasSelection))
            {
        ...
        private bool DrawEnvironmentSceneEditor(Rect rect, EnvironmentPromptConfig envConfig)
        {
            if (_selectedEnvironmentSceneIndex < 0 || _selectedEnvironmentSceneIndex >= envConfig.SceneEntries.Count)
            {
                ...
                Widgets.Label(rect, "RimDiplomacy_EnvironmentSelectSceneHint".Translate());
                GUI.color = Color.white;
                return false;
            }
            ...
            Widgets.Label(new Rect(rect.x, y, rect.width, 18f), "RimDiplomacy_EnvironmentSceneNameLabel".Translate());
            y += 18f;
            ...
            Widgets.Label(new Rect(rect.x, y, 90f, 18f), "RimDiplomacy_EnvironmentPriorityLabel".Translate());
            y += 18f;
            ...
            changed |= DrawEnvironmentCheckbox(
                ...
                "RimDiplomacy_EnvironmentEntryEnabled",
                ref entry.Enabled);
            y += 24f;
            changed |= DrawEnvironmentCheckbox(
                new Rect(rightX, y, rect.width - 110f, 24f),
                "RimDiplomacy_EnvironmentApplyDiplomacy",
                ref entry.ApplyToDiplomacy);
            y += 24f;
            changed |= DrawEnvironmentCheckbox(
                new Rect(rightX, y, rect.width - 110f, 24f),
                "RimDiplomacy_EnvironmentApplyRPG",
                ref entry.ApplyToRPG);
            y += 28f;

            Widgets.Label(new Rect(rect.x, y, rect.width, 18f), "RimDiplomacy_EnvironmentSceneTagsLabel".Translate());
            y += 18f;
            ...
            Widgets.Label(new Rect(rect.x, y, rect.width, 18f), "RimDiplomacy_EnvironmentSceneContentLabel".Translate());
            y += 20f;
        ...
        {
            Rect contentRect = DrawEnvironmentCard(rect, "RimDiplomacy_EnvironmentRpgParamsLabel");
        ...
        {
            Rect contentRect = DrawEnvironmentCard(rect, "RimDiplomacy_EnvironmentPreviewTitle");
            Rect refreshRect = new Rect(contentRect.xMax - 136f, contentRect.y, 136f, 24f);
            if (Widgets.ButtonText(refreshRect, "RimDiplomacy_EnvironmentPreviewRefresh".Translate()))
            {
                _environmentPreviewCooldown = 0;
            }
        ...
        private string BuildEnvironmentPreviewText()
        {
            try
            {
                ...
                if (sb.Length == 0)
                {
                    sb.AppendLine("RimDiplomacy_EnvironmentPreviewNoContext".Translate());
                }
