    ...
    public static class DiplomacyEventManager
    {
        public static bool TriggerCaravanEvent(Faction faction, CaravanType caravanType)
        {
            try
            {
                ...
                if (map == null)
                {
                    Log.Warning("[RimDiplomacy] No player home map found for caravan event");
                    return false;
                }
                ...
                if (success)
                {
                    Log.Message($"[RimDiplomacy] Triggered {caravanType} caravan from {faction.Name}");
                }
                else
                {
                    Log.Warning($"[RimDiplomacy] Failed to trigger {caravanType} caravan from {faction.Name}");
                }
            ...
            {
                Log.Error($"[RimDiplomacy] Error triggering caravan event: {ex}");
                return false;
            }
        }
        ...
        private static TraderKindDef GetTraderKindForType(Faction faction, CaravanType caravanType)
        {
            ...
            Log.Message($"[RimDiplomacy] Found {matchingTraders.Count} matching traders for {caravanType}");
            foreach (var t in matchingTraders)
            {
                Log.Message($"[RimDiplomacy]   - {t.defName}");
            }
            
            if (matchingTraders.Count > 0)
            {
                var selected = matchingTraders.RandomElement();
                Log.Message($"[RimDiplomacy] Selected trader: {selected.defName}");
                return selected;
            }
            
            Log.Warning($"[RimDiplomacy] No matching traders found for {caravanType}, using null");
            return null;
        }
        ...
        public static bool TriggerAidEvent(Faction faction, AidType aidType)
        {
            try
            {
                ...
                if (map == null)
                {
                    Log.Warning("[RimDiplomacy] No player home map found for aid event");
                    return false;
                }
            ...
            {
                Log.Error($"[RimDiplomacy] Error triggering aid event: {ex}");
                return false;
            }
        }
        ...
        private static bool TriggerMilitaryAid(Faction faction, Map map)
        {
            ...
            if (militaryAidDef != null && militaryAidDef.Worker != null)
            {
                ...
                if (success)
                {
                    Log.Message($"[RimDiplomacy] Triggered military aid from {faction.Name}");
        ...
        private static bool TriggerMedicalAid(Faction faction, Map map)
        {
            ...
            Log.Message($"[RimDiplomacy] Triggered medical aid from {faction.Name}");
            return true;
        }
        ...
        private static bool TriggerResourceAid(Faction faction, Map map)
        {
            ...
            Log.Message($"[RimDiplomacy] Triggered resource aid from {faction.Name}");
            return true;
        }
        ...
        {
            int baseTicks = isAid 
                ? (RimDiplomacyMod.Instance?.InstanceSettings?.AidDelayBaseTicks ?? 90000)
                : (RimDiplomacyMod.Instance?.InstanceSettings?.CaravanDelayBaseTicks ?? 135000);
        ...
        public static bool ScheduleDelayedCaravan(Faction faction, CaravanType caravanType)
        {
            try
            {
                ...
                Log.Message($"[RimDiplomacy] Scheduled delayed caravan from {faction.Name}, type={caravanType}, delay={delayDays:F1} days");
                return true;
            }
            catch (Exception ex)
            {
                Log.Error($"[RimDiplomacy] Error scheduling delayed caravan: {ex}");
                return false;
            }
        }
        ...
        public static bool ScheduleDelayedAid(Faction faction, AidType aidType)
        {
            try
            {
                ...
                Log.Message($"[RimDiplomacy] Scheduled delayed aid from {faction.Name}, type={aidType}, delay={delayDays:F1} days");
                return true;
            }
            catch (Exception ex)
            {
                Log.Error($"[RimDiplomacy] Error scheduling delayed aid: {ex}");
                return false;
            }
        }
