...
namespace RimDiplomacy.DiplomacySystem
{
    ...
    public partial class GameComponent_DiplomacyManager
    {
        ...
        private void InitializeSocialCircleOnNewGame()
        {
            ...
            socialCircleState.NextPostTick = currentTick + SocialCircleService.CalculateNextIntervalTicks(RimDiplomacyMod.Instance?.InstanceSettings);
        }
        ...
        {
            return RimDiplomacyMod.Instance?.InstanceSettings?.EnableSocialCircle ?? true;
        }
            ...
            DebugGenerateReason reason = DebugGenerateReason.DialogueExplicit)
        {
            ...
            if (isFromPlayerDialogue && !(RimDiplomacyMod.Instance?.InstanceSettings?.EnablePlayerInfluenceNews ?? true)) return false;
        ...
        {
            if (sourceFaction == null || sourceFaction.defeated) return false;
            if (!(RimDiplomacyMod.Instance?.InstanceSettings?.EnablePlayerInfluenceNews ?? true)) return false;
            ...
            string summary = "RimDiplomacy_SocialPostSummaryFromDialogue".Translate();
        ...
        private void TryGenerateScheduledSocialPost(int currentTick)
        {
            ...
            if (!created)
            {
                Log.Warning("[RimDiplomacy] Scheduled social post generation skipped due to missing eligible faction.");
            }
        }

        private bool TryGenerateAutomaticPost(DebugGenerateReason reason, int currentTick, bool bypassSimulationToggle)
        {
            if (!bypassSimulationToggle && !(RimDiplomacyMod.Instance?.InstanceSettings?.EnableAISimulationNews ?? true))
            {
                return false;
            }
        ...
        private bool EnqueuePublicPostInternal(PublicSocialPost post, DebugGenerateReason reason)
        {
            ...
            {
                Log.Message($"[RimDiplomacy] Skipped duplicate social post ({reason}).");
                return false;
            }
