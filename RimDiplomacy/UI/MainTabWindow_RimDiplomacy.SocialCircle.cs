...
namespace RimDiplomacy.UI
{
    ...
    /// </summary>
    public partial class MainTabWindow_RimDiplomacy
    {
        private enum RimDiplomacyMainTab
        {
            Factions,
            SocialCircle
        }

        private RimDiplomacyMainTab currentMainTab = RimDiplomacyMainTab.Factions;
        ...
        private void DrawMainTabButtons(Rect headerRect)
        {
            ...
            string socialLabel = unreadCount > 0
                ? "RimDiplomacy_SocialCircleTabWithUnread".Translate(unreadCount)
                : "RimDiplomacy_SocialCircleTab".Translate();
            ...
            DrawMainTabButton(factionsRect, "RimDiplomacy_FactionsTab".Translate(), currentMainTab == RimDiplomacyMainTab.Factions, RimDiplomacyMainTab.Factions);
            DrawMainTabButton(socialRect, socialLabel, currentMainTab == RimDiplomacyMainTab.SocialCircle, RimDiplomacyMainTab.SocialCircle);
        }

        private void DrawMainTabButton(Rect rect, string label, bool active, RimDiplomacyMainTab targetTab)
        {
        ...
        private void SetMainTab(RimDiplomacyMainTab tab)
        {
            if (currentMainTab == tab) return;
            currentMainTab = tab;
            if (tab == RimDiplomacyMainTab.SocialCircle)
            {
        ...
        private void DrawSocialCirclePanel(Rect rect)
        {
            ...
            if (manager == null)
            {
                Widgets.Label(rect.ContractedBy(12f), "RimDiplomacy_SocialUnavailable".Translate());
                return;
            }
        ...
        private void DrawSocialToolbar(Rect rect, GameComponent_DiplomacyManager manager)
        {
            ...
            DrawFilterButton(allRect, "RimDiplomacy_SocialFilterAll".Translate(), !socialCategoryFilter.HasValue, null);
            DrawFilterButton(militaryRect, "RimDiplomacy_NewsCategoryMilitary".Translate(), socialCategoryFilter == SocialPostCategory.Military, SocialPostCategory.Military);
            DrawFilterButton(ecoRect, "RimDiplomacy_NewsCategoryEconomic".Translate(), socialCategoryFilter == SocialPostCategory.Economic, SocialPostCategory.Economic);
            DrawFilterButton(dipRect, "RimDiplomacy_NewsCategoryDiplomatic".Translate(), socialCategoryFilter == SocialPostCategory.Diplomatic, SocialPostCategory.Diplomatic);
            DrawFilterButton(anomalyRect, "RimDiplomacy_NewsCategoryAnomaly".Translate(), socialCategoryFilter == SocialPostCategory.Anomaly, SocialPostCategory.Anomaly);

            int total = manager.GetSocialPosts(999).Count;
            Text.Anchor = TextAnchor.MiddleRight;
            GUI.color = TextSecondary;
            Widgets.Label(countRect, "RimDiplomacy_SocialTotalPosts".Translate(total));
            GUI.color = Color.white;
        ...
        private void DrawSocialPostList(Rect rect, GameComponent_DiplomacyManager manager)
        {
            ...
            if (posts.Count == 0)
            {
                Widgets.Label(rect, "RimDiplomacy_SocialNoPosts".Translate());
                return;
            }
        ...
        private void DrawSocialPostCard(Rect rect, PublicSocialPost post)
        {
            ...
            string sourceName = post.SourceFaction?.Name ?? "RimDiplomacy_Unknown".Translate();
            string targetName = post.TargetFaction?.Name ?? "RimDiplomacy_SocialNoTarget".Translate();
            ...
            GUI.color = TextSecondary;
            Widgets.Label(headerRect, "RimDiplomacy_SocialPostHeader".Translate(sourceName, targetName, categoryLabel, credibility));
            ...
            Widgets.Label(footerLeft, "RimDiplomacy_SocialPostEffectLine".Translate(post.EffectSummary ?? string.Empty));
        ...
        private string FormatSocialPostTime(int tick)
        {
            ...
            if (days < 1f)
            {
                ...
                return "RimDiplomacy_SocialHoursAgo".Translate(hours.ToString("F1"));
            }
            return "RimDiplomacy_SocialDaysAgo".Translate(days.ToString("F1"));
        }
    }
}
