...
namespace RimDiplomacy
{
    ...
    {
        private const string PROMPT_DIRECTORY = "RimDiplomacy";
        ...
        public static void EnsureDirectoryExists()
        {
            try
            {
                if (!Directory.Exists(BasePath))
                {
                    ...
                    Log.Message($"[RimDiplomacy] 创建 prompt 目录：{BasePath}");
                }
            }
            catch (Exception ex)
            {
                Log.Error($"[RimDiplomacy] 创建 prompt 目录失败：{ex.Message}");
            }
        }
        ...
        public static PromptConfig LoadGlobalPrompt()
        {
            try
            {
                ...
                if (File.Exists(GlobalPromptPath))
                {
                    ...
                    {
                        Log.Message($"[RimDiplomacy] 从文件加载全局 Prompt: {config.Name}");
                        return config;
                    }
                }
                
                Log.Message($"[RimDiplomacy] Prompt 文件不存在或无效，使用默认 Prompt");
                return CreateDefaultPromptConfig();
            }
            catch (Exception ex)
            {
                Log.Error($"[RimDiplomacy] 加载 Prompt 文件失败：{ex.Message}");
                return CreateDefaultPromptConfig();
            }
        }
        ...
        private static PromptConfig ParseJsonToPromptConfig(string json)
        {
            ...
            {
                Log.Warning($"[RimDiplomacy] JSON 解析失败：{ex.Message}");
                return null;
            }
        }
        ...
        public static void SaveGlobalPrompt(PromptConfig config)
        {
            try
            {
                ...
                if (config == null)
                {
                    Log.Warning("[RimDiplomacy] 尝试保存空的 Prompt 配置");
                    return;
                }
                ...
                Log.Message($"[RimDiplomacy] 已保存全局 Prompt 到：{GlobalPromptPath}");
            }
            catch (Exception ex)
            {
                Log.Error($"[RimDiplomacy] 保存 Prompt 文件失败：{ex.Message}");
            }
        }
        ...
        public static void DeleteCustomPrompt()
        {
            try
            {
                if (File.Exists(GlobalPromptPath))
                {
                    ...
                    Log.Message("[RimDiplomacy] 已删除自定义 Prompt，将使用默认 Prompt");
                }
            }
            catch (Exception ex)
            {
                Log.Error($"[RimDiplomacy] 删除 Prompt 文件失败：{ex.Message}");
            }
        }
    }
}
